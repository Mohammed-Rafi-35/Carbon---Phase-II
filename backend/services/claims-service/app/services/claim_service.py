...
class ClaimService:
    ...
    def auto_create_claim(self, *, user_id: UUID, event_id: UUID, event_payload: dict | None = None) -> Claim:
        existing = self.repo.get_by_user_event(user_id=user_id, event_id=event_id)
        if existing:
            return existing

        claim = self.repo.create(user_id=user_id, event_id=event_id, status=ClaimStatus.INITIATED)
        self._log(
            claim,
