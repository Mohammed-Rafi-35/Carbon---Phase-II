...
class Settings(BaseSettings):
	...
	database_url: str = Field(default="postgresql+psycopg://postgres:postgres@localhost:5432/notification_db")
	database_connect_timeout_seconds: int = Field(default=5)

	rabbitmq_url: str = Field(default="amqp://guest:guest@localhost:5672//")
	redis_url: str | None = Field(default="redis://localhost:6379/0")
	enable_event_bus_consumer: bool = Field(default=True)
	notification_event_queue_name: str = Field(default="notification.events")
	notification_event_routing_key: str = Field(default="#")
